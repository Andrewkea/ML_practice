...
# -------------------------
# Конфигурация путей (относительные)
# -------------------------
# Базовый путь - директория со скриптом
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = os.path.join(BASE_DIR, 'data')

# Города по умолчанию (можно переопределить через конфиг)
DEFAULT_CITIES = {
    'EKB': 'Екатеринбург',
    'NN': 'Нижний Новгород', 
    'Vladimir': 'Владимир',
...
class DataProcessor:
    ...
    def __init__(self, data_dir: str = None, cities_config: Dict = None):
        """
        Инициализация процессора данных.
        
        Args:
            data_dir: Путь к директории с данными (по умолчанию: ./data)
            cities_config: Словарь с кодами и названиями городов
        """
        if data_dir is None:
            data_dir = DATA_DIR
        ...
        if not os.path.exists(data_dir):
            ...
            print("\nPlease place your CSV files in this directory:")
            print("Expected format:")
            print(f"  - {os.path.join(data_dir, 'CITYCODE_images.csv')}")
            print(f"  - {os.path.join(data_dir, 'CITYCODE_places.csv')}")
            print("\nExample:")
            print(f"  - {os.path.join(data_dir, 'EKB_images.csv')}")
            print(f"  - {os.path.join(data_dir, 'EKB_places.csv')}")
            print()
        ...
        print(f"Processing cities: {list(self.cities.keys())}")
    ...
    def process_city(self, city_code: str, city_name: str = None) -> pd.DataFrame:
        ...
        if city_name is None:
            city_name = self.cities.get(city_code, city_code)
        ...
        if not os.path.exists(images_csv):
            ...
            print(f"Expected file name: {city_code}_images.csv")
        ...
        try:
            ...
            if os.path.exists(places_csv):
                ...
                print(f"Continuing with image data only...")
                # Возвращаем только данные об изображениях
                return self._add_empty_place_columns(df_images)
    ...
    def process_all_cities(self) -> pd.DataFrame:
        ...
        for city_code in self.cities.keys():
        ...
        if all_data:
            ...
            print("\nNo data processed!")
            print(f"\nMake sure you have placed CSV files in {self.data_dir}")
            print("Expected format: CITYCODE_images.csv and CITYCODE_places.csv")
            print(f"Example: {os.path.join(self.data_dir, 'EKB_images.csv')}")
            return pd.DataFrame()
...
def main():
    ...
    if args.command == 'process':
        ...
        if not os.path.exists(model_path):
            ...
            print(f"Please build the system first using: python landmark_system.py build")
            return
        ...
        if not os.path.exists(query_img_path):
            ...
            query_img_path = os.path.join(os.getcwd(), args.image)
            if not os.path.exists(query_img_path):
                print(f"Error: Cannot find query image at {args.image}")
                print(f"Tried: {args.image}")
                print(f"Tried: {query_img_path}")
                return
        
        query_img = read_image_from_path(query_img_path)
        if query_img is None:
            print(f"Error: Cannot load query image from {query_img_path}")
            return
        ...
        parser.print_help()
        print("\n" + "="*60)
        print("LANDMARK SEARCH SYSTEM - QUICK START")
        print("="*60)
        print("\nSetup:")
        print("  1. Create a 'data' directory or specify with --data_dir")
        print("  2. Place CSV files in data directory:")
        print("     - CITYCODE_images.csv (e.g., EKB_images.csv)")
        print("     - CITYCODE_places.csv (e.g., EKB_places.csv)")
        
        print("\nWorkflow:")
        print("  1. python landmark_system.py process --data_dir ./data")
        print("  2. python landmark_system.py build --dataset_csv combined_dataset.csv --clean")
        print("  3. python landmark_system.py search_image --image query.jpg --top_k 5")
        print("  4. python landmark_system.py search_text --query 'Эрмитаж' --top_k 5")
        
        print("\nExamples with custom data directory:")
        print("  python landmark_system.py --data_dir /path/to/your/data process")
        print("  python landmark_system.py --data_dir /path/to/your/data build --clean")
